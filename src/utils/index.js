...
export const showField = (fieldName) => {
  return !validationFields ||
    (!validationFields.loading && !validationFields.fields?.checkout?.[fieldName]) ||
    (!validationFields.loading && validationFields.fields?.checkout?.[fieldName] &&
      validationFields.fields?.checkout?.[fieldName]?.enabled)
}
export const getFixedFromPercentage = (percentage, price) => {
  let rate = percentage * price / 100
  return rate;
}

export const getOrderTypetext = (orderTypes = []) => {
  let str = '';
...
export const getOrderTypeInt = (str) => {
  let int = 0;
  if (str == 'delivery') {
    int = 1;
  }
