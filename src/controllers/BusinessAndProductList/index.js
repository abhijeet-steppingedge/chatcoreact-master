...
import { useConfig, useEvent } from 'ordering-components'
...
export const BusinessAndProductList = (props) => {
  ...
  const getProducts = async () => {
    ...
    categoryState.products.forEach(product => {
  ...
  const getBusiness = async () => {
    try {
      ...
      if (shop?.menu && Object.keys(shop?.menu).length > 0) {
      ...
      if (result.menus && result.menus.length) {
        result.menus = result.menus.filter(menu => menu.enabled == true)
      }
      const data = {
        ...businessState,
        business: result,
