...
export const PaymentOptions = (props) => {
  ...
  const parsePaymethods = (paymethods) => {
    const _paymethods = paymethods && paymethods
      .filter(credentials => isCustomerMode
        ...
        :
        ((!orderState.loading && credentials?.allowed_order_types && Object.keys(credentials?.allowed_order_types).length > 0
          && Object.values(credentials?.allowed_order_types).includes(orderState?.options?.type)) || credentials?.allowed_order_types === null)
        && !paymethodsNotAllowed.includes(credentials?.paymethod?.gateway))
      .map(credentials => {
        return {
  ...
  const handlePaymethodDataChange = (data) => {
    ...
    if (Object.keys(data).length) {
      ...
      const paymethod = isOpenMethod.paymethod || props.paySelected;
      setPaymethodsSelected(paymethod)
  ...
  useEffect(() => {
    const methods = parsePaymethods(paymethods);
    setPaymethodsList({ paymethods: methods, loading: false })
