...
const CustomStyleOrderTypeSelectorHeaderUI = (props) => {
  const {
    ...
    <OrderTypeWrapper>
      {
        orderTypeStyle
          ? (
            <OrderTypeBoxGroup>
              {
                orderTypes && orderTypes.filter(type => configTypes?.includes(type.value)).map((orderType) => {
                  return (
                    <OrderItem key={orderType.value} onClick={() => handleChangeOrderType(orderType.value)}>
                      <OrderItemWraper active={optionType == orderType.value ? true : false}>
...
export const CustomStyleOrderTypeSelectorHeader = (props) => {
  const [, t] = useLanguage()
