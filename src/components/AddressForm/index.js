...
import { GoogleGpsButton } from 'ordering-ui'
...
const inputNames = [
  ...
  { name: 'internal_number', code: 'Unit Number' },
  { name: 'zipcode', code: 'Pincode' },
...
const AddressFormUI = (props) => {
  ...
  const onSubmit = async () => {
    ...
    const response = await fetch(`https://apiv4.ordering.co/${systemConfig.api.version}/${systemConfig.api.language}/${systemConfig.project}/business/${business.slug}?location=${formState?.changes?.location?.lat},${formState?.changes?.location?.lng}`, requestOptions)
          ...
          {inputNames.map(field => showField && showField(field.name) && (
            field.name === 'address' ? (
              <React.Fragment key={field.name}>
                ...
                {((addressState?.address?.location && Object.keys(addressState?.address?.location).length > 0) || (formState?.changes?.location && Object.keys(formState?.changes?.location).length > 0)) && !toggleMap && (
              ...
              <React.Fragment key={field.name}>
                {field.name !== 'address_notes' ? (
                  <Input
                    ...
                    autoComplete='new-field'
                    maxLength={30}
                    style={{ width: '45%', marginLeft: "2px", marginRight: "2px" }}
                  />
                ) : (
                  <TextArea
                    rows={1}
