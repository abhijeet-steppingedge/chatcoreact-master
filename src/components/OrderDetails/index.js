...
import { OrderDetails as OrderDetailsController } from '../../controllers/OrderDetails'
...
import { ProductItemAccordion } from 'ordering-ui'
import { Modal } from 'ordering-ui'
import { ReviewOrder } from 'ordering-ui'
import { ProductShare } from 'ordering-ui'
import { verifyDecimals } from '../../utils'
import { ReviewProduct } from 'ordering-ui'
import { ReviewDriver } from 'ordering-ui'
...
const OrderDetailsUI = (props) => {
  ...
  useEffect(() => {
    if (businessData && Object.keys(businessData).length) {
      localStorage.setItem('previous-business', businessData?.slug)
    }
  }, [businessData])
  useEffect(() => {
    order && Object.keys(order).length ? formatCurrentTime(order?.delivery_datetime) : null
  }, [order])
            ...
            <LeftContainer>
              <OrderInfo>
                <div>
                  ...
                  <OrderStatus>
                    <span style={{ fontWeight: 'bold' }}>{getOrderType(order?.delivery_type)}</span>
                  </OrderStatus>
                </div>
              </OrderInfo>
              ...
              <OrderCustomer>
                ...
                {order?.customer?.internal_number && (
                  <p>{order?.customer?.internal_number}</p>
                )}
                {order?.customer?.tag && (
                  <p>{order?.customer?.tag}</p>
                )}
              </OrderCustomer>
